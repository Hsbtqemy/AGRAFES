...
# Réexport : le recorder vit dans services/ depuis ACT-01 (la CLI en a besoin et ne
# peut pas importer ce module), mais huit appels d'ici et deux tests le nomment
# `sidecar.make_resegment_recorder`. Le nom public ne bouge pas.
from .services.segment_service import make_resegment_recorder  # noqa: F401
...
class _CorpusHandler(BaseHTTPRequestHandler):
    ...
    def _send_json(self, data: dict, status: int = 200) -> None:
        ...
        try:
            ...
            logger.warning(
                "Client disconnected before response could be sent (%s %s)",
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            logger.warning(
                "Client disconnected during do_POST (%s) — server continues",
                self.path,
            )
        except Exception as exc:
    ...
    def _handle_import(self, body: dict) -> None:
        # Thin adapter over the import service (audit P0-1). The service owns the
        # validation + import + run + family-relation logic; this handler owns the
        # write-lock and the HTTP envelope, mapping ServiceError to wire codes.
        ...
        try:
            with self._lock():
                result = import_document(self._conn(), body)
        except ValidationError as exc:
    ...
    def _ensure_document_workflow_columns(self) -> None:
        ...
        with self._lock():
            ...
            if "work_title" not in cols:
                self._conn().execute("ALTER TABLE documents ADD COLUMN work_title TEXT")
            if "pub_place" not in cols:
                self._conn().execute("ALTER TABLE documents ADD COLUMN pub_place TEXT")
            if "publisher" not in cols:
                self._conn().execute("ALTER TABLE documents ADD COLUMN publisher TEXT")
            self._conn().execute(
            ...
            success_payload(
                {
                    "doc": {
                        ...
                        "work_title": doc_row[14],
                        "pub_place": doc_row[15],
                        "publisher": doc_row[16],
                    },
                    "lines": lines,
                    "count": len(lines),
