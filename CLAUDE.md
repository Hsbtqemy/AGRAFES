...
- `sidecar.py` (**10 517 lines**, measured 2026-08-26 — it was ~8.5k when this line was written; the file grows, see the growth gate below) is an `http.server` handler class. `do_POST` has an **explicit allowlist of write paths** (`_write_paths`) — new POST routes must be added there; writes go through `with self._lock()`.
