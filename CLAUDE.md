...
- **ShareDocs / WebDAV ingestion** (`remote/`, `docs/DESIGN_sharedocs_ingestion.md`): Phases 1-4 are merged to `dev` — CLI `import-remote` (P1), sidecar routes `POST /webdav/list` + `POST /import-remote` (P2), Prep UI screen (P3), and credential-keychain / root-prefill / multi-select cart / batch-undo (P4A-D). **Phase 5** (per-file format+language detection via the shared `importDetect` module — single source of truth for local Import **and** ShareDocs) **+ its follow-up** (checked-folder PROPFIND expansion in the cart via `routeEntriesToImport`/`dedupeDetectedFiles`; local-import TEI `xml:lang` convergence via `detectLanguageForMode`) are **built** on `feat/sharedocs-p5-detection` — see `docs/DESIGN_sharedocs_ingestion.md` §11/§11.8 + `docs/TICKET_SHAREDOCS_INGESTION_P5_DETECTION.md`. **Phase 6** (families: source↔translation link) is deferred. The DB is never shared over WebDAV — this is ingestion-only.
