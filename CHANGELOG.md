...
- **shell / sidecar — fuite de process `multicorpus.exe` orphelins (R-01)** : ouvrir une DB en mauvais état (WAL périmé, dossier cloud-sync type OneDrive qui verrouille le fichier SQLite) figeait le sidecar et laissait s'accumuler des process orphelins que la machinerie de cleanup ne récupérait pas. Trois causes corrigées (P0+P1) et une différée (P2), cf. `docs/AUDIT_FOLLOW_UP.md` R-01 et `docs/DECISIONS.md` ADR-042 : **(R-01a)** le shutdown gracieux côté Rust envoyait le mauvais header de token (`X-Sidecar-Token` au lieu de `X-Agrafes-Token`) → `/shutdown` 401 → fuite à **chaque** fermeture ; corrigé. **(R-01c)** côté client (`shared/sidecarCore.ts`) : **single-flight** sur `ensureRunning` (une seule tentative de spawn concurrente par DB — supprime le « spawn-storm » des 4 requêtes parallèles de l'écran Constituer) + kill du child sur tout échec de spawn. **(R-01d)** `register_sidecar` transmet le `pid` et le Rust **force-kill par PID** (`taskkill /F /T` / `kill -9`) en secours si le `/shutdown` gracieux échoue/timeout. **(R-01b — différé)** rendre `/health`/`/shutdown` non-bloquants sous un handler figé impliquerait de passer le serveur en `ThreadingHTTPServer`, mais plusieurs lectures partagent la connexion SQLite **hors lock** (sûres seulement parce que le serveur est mono-thread) → reporté à un chantier dédié (sérialisation au dispatch via `RLock` ou connexions par thread) ; mitigé entre-temps par R-01c + R-01d. Aucun endpoint ni `sidecar.py` modifié (contrat OpenAPI inchangé). Tests : 2 vitest (single-flight + reap on failed spawn) ; vérifié en live (1 spawn propre à l'ouverture du démo, 0 orphelin après fermeture avec sidecar actif) + smoke headless du binaire (header `/shutdown`).
