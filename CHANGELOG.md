...
- **Lot P0 de l'audit général 2026-06-28** (suite) :
  ...
  - **SID-02 — bypass d'authentification sur `/jobs`** : `POST /jobs` enqueue des jobs `index`/`curate`/`segment` (mutateurs DB) mais le chemin `/jobs` était **absent de `_write_paths`** → aucun token requis même sous `--token auto`. Ajouté à l'allowlist d'écriture.
  - **QRY-02 — injection de formule CSV** : `export_csv` n'échappait pas les cellules commençant par `= + - @` ou un caractère de contrôle (titres/textes de corpus attaquant-contrôlables, exécution à l'ouverture dans Excel/LibreOffice). Neutralisation par préfixe `'` (les tirets typographiques `— –` ne sont pas touchés). Test : `test_exporters.py::test_csv_export_neutralizes_formula_injection`.
