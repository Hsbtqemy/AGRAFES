...
export class AlignPanel {
  ...
  // Collisions sub-feature — extracted to AlignCollisionPanel (U-02)
  private _collisionPanel!: AlignCollisionPanel;
  ...
  render(): HTMLElement {
    ...
    this._el = el;
    this._collisionPanel = new AlignCollisionPanel(el, {
      getConn: () => this._conn(),
      log: (m, e) => this._cb.log(m, e),
      toast: (m, e) => this._cb.toast(m, e),
      onCollisionCountChanged: (n) => {
        this._lastCollisionCount = n;
        if (this._el) this._updateTopbarKpi(this._el);
      },
    });
  ...
  private _updateTopbarKpi(el: HTMLElement): void {
    ...
    kpi.querySelector("#align-coll-toggle")?.addEventListener("click", () => {
      ...
      if (!isOpen) {
        section.scrollIntoView({ behavior: "smooth", block: "nearest" });
        void this._collisionPanel?.loadCollisions();
      }
    });
  }
