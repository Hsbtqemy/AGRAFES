...
console.log("─── formatTelemetryStats — empty ───");
{
  const s = aggregateTelemetry([], 0);
  const txt = formatTelemetryStats(s);
  assert(txt.includes("Aucun événement"), "empty case mentions 'Aucun événement'");
  assert(!txt.includes("Total :"), "no Total line when 0 events");
}

console.log("─── formatTelemetryStats — empty + parse errors ───");
{
  const s = aggregateTelemetry([], 3);
  const txt = formatTelemetryStats(s);
  assert(txt.includes("3 ligne(s) malformée"), "parse errors mentioned even when no events");
}

console.log("─── formatTelemetryStats — populated ───");
{
  const records = [
  ...
  const s = aggregateTelemetry(records, 0);
  const txt = formatTelemetryStats(s);
  assert(txt.includes("Total : 4 événement"), "Total count present");
  assert(txt.includes("Période :"), "Période line present when ts available");
  assert(txt.includes("Répartition par event"), "by_event section header");
  assert(txt.includes("stage_completed"), "stage_completed listed");
  assert(txt.includes("Stages (durée moyenne"), "stages section present");
  assert(txt.includes("import") && txt.includes("1500 ms"), "import row with duration");
  assert(txt.includes("curate") && txt.includes("0/1"), "curate showing 0/1 success");
  assert(txt.includes("Caps atteints"), "caps section");
  assert(txt.includes("curate_preview_5000"), "cap name listed");
  assert(txt.includes("Top erreurs"), "top errors section");
  assert(txt.includes("SidecarError"), "error class listed");
}

console.log("─── formatTelemetryStats — sections présentes uniquement si non vides ───");
{
  // Only by_event, no stages/caps/errors
  const records = [
    { ts: "2026-04-29T10:00:00.000Z", event: "doc_deleted", doc_id: 1, had_curation: false, had_alignment: false },
  ];
  const s = aggregateTelemetry(records, 0);
  const txt = formatTelemetryStats(s);
  assert(txt.includes("doc_deleted"), "doc_deleted mentioned in by_event");
  assert(!txt.includes("Stages"), "no stages section when no stage_completed");
  assert(!txt.includes("Caps atteints"), "no caps section when no cap_hit");
  assert(!txt.includes("Top erreurs"), "no errors section when no error_user_facing");
}

console.log("─── formatTelemetryStats — sortie déterministe (tri par count) ───");
{
  const records = [
  ...
  const s = aggregateTelemetry(records, 0);
  const txt = formatTelemetryStats(s);
  // c (3) doit apparaître avant b (2) avant a (1)
  const cPos = txt.indexOf("\n  c ");
  const bPos = txt.indexOf("\n  b ");
  const aPos = txt.indexOf("\n  a ");
  assert(cPos < bPos && bPos < aPos, `events ordered desc by count : c=${cPos} b=${bPos} a=${aPos}`);
}
