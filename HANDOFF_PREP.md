...
| **SegmentationView** | — | **Retirée** (voie a, tranches 1-6 ; voir `docs/DESIGN_segmentation_retirement.md`) — la segmentation est désormais une **couche du canvas** (`components/SegmentPane.ts`) | Structure matcher décomposé (édition→canvas Segment ; appariement→Alignement ; propagate=primitive famille-pilotée de couture). Onglet « Rôles » = couche Rôles du canvas (`RolesPane`). Undo Mode A de l'assignation de rôle ajouté (mig 033). `SegmentationView.ts` + modules exclusifs supprimés ; CSS mort purgé (`aa7ded3`). | — |
| **CurationView** | — | **Retirée (R6.5-C)** — la curation est désormais une **couche du canvas** (`components/CurationPane.ts`) | Parité au canvas atteinte en R6.5-B (exceptions/relu/Find-Replace/undo/toggle non_traduit) avant le retrait du legacy. CSS mort purgé en fin de migration (`aa7ded3`, avec celui de Segmentation). | — |
| **AlignPanel** | 1971 | Mature | Famille review mode rodé. Collisions encore complexes à résoudre pour un débutant. | Moyenne |
| **AnnotationView** | — | **Retirée** — l'annotation est désormais une **couche du canvas** (`components/AnnotationPane.ts`, R5.2/R6.5-A) | Étape finale avant le concordancier ; relecture/vérification rapide. Temps court = **signal positif** (workflow efficient), pas un manque d'engagement — les frictions sont en amont (Import/Segmentation/Curation). | Faible en durée, indispensable en finalité |
...
| **ActionsScreen** | 847 | Hub/dispatcher post-refactor | Les écrans legacy Curation/Segmentation/Annotation ont été **retirés** (couches du canvas) ; ActionsScreen dispatche vers le canvas + reste utile pour reindex FTS. `segFocusDocRoles`/`focusSegmentationOnUnit` routent désormais vers le canvas (couches Rôles/Segment). | Faible |
...
1. ~~**DOCX bilingue en tableau 2-colonnes non lu par l'importer**~~ ✅ **Fait post-v0.1.43** : nouveau paramètre `column_index` sur `import_docx_numbered_lines` ([docx_numbered_lines.py](src/multicorpus_engine/importers/docx_numbered_lines.py), commits `959930d` + `756584a` + `c5abb9f` pour le dedup vMerge). Quand fourni (entier ≥ 1), itère le body en ordre du document et extrait la cellule de cette colonne (1-based) de chaque table — paragraphes aplatis. Cas pathologiques gérés (table plus étroite, cellule fusionnée H et V, sous-table imbriquée, colonne sans `[N]` dominante, 0 unité extraite) → compteurs dédiés dans `ImportReport` + warnings actionnables, plus de « 0 line units » silencieux. UI : champ optionnel « col » dans ImportScreen, visible uniquement pour `docx_numbered_lines`. Pas de famille auto-créée. 10 tests pytest. **Périmètre limité** : `docx_paragraphs`, `odt_numbered_lines`, `odt_paragraphs` ont des frictions analogues mais différentes (cf. `docs/IMPORT_DOCX_ODT_UNICODE_QA.md` § 8 « Périmètre V1 ») — non corrigées, à reprendre si signal explicite.
...
3. ~~**Pas de feedback « étape suivante »** entre les écrans.~~ ✅ **Adressé post-v0.2.3** : bandeau « étape suivante » contextuel. Après une action de pipeline réussie (curation appliquée, segmentation validée, alignement terminé), un bandeau bleu non-bloquant en fin de sous-vue propose l'étape suivante logique + bouton de navigation. Les suggestions sont calculées à partir de l'état réel observé (index FTS périmé, présence de traductions) — voir `lib/prepNextStep.ts` (`computeNextSteps`, helper pur, 10 tests Vitest). Le bandeau *suggère* sans contraindre (HANDOFF §3 : aucun ordre n'est bloquant) et est masquable. Un fil d'Ariane d'ensemble persistant (strip d'état sur le hub) reste une option non retenue pour cette itération.

4. ~~**Reindex FTS oublié**.~~ ✅ **Adressé** (v0.2.2 + v0.2.5). v0.2.2 : chip « ⚠ Index » dans MetadataScreen sur chaque doc dont l'index FTS est périmé, staleness dérivée en direct (`indexer.stale_doc_ids`, pas de flag persisté). v0.2.5 (backlog F4) : la vue Documents devient le point d'arbitrage unique — bouton d'en-tête « Mettre à jour l'index » avec état global agrégé (« ✓ Index à jour » / « ⚠ Mettre à jour l'index (N) »), chip « ⚠ Index » rendu cliquable, et réglage opt-in « Auto après curation » (réindexation automatique non bloquante après une curation qui périme l'index). Helper pur `lib/prepIndexStatus.ts`.

5. **CurationView 3651 lignes** (3795 avant la décomposition Phase 1-5e). Friction réduite pour les contributeurs grâce à l'extraction en 9 modules purs sous `lib/curation*.ts`. ~~L'écran reste lent à charger sur les gros corpus — pas de virtualization du DOM.~~ **Correction post-v0.2.3** : il n'y a pas de DOM non-borné. La liste de diff est paginée (`CURATE_PAGE_SIZE = 50`, une page rend 50 `<tr>` max), la file de documents ne rend que 3 items. Aucune « lenteur de chargement » n'a été mesurée côté frontend ; si elle existe, elle vient du backend (`/curate` rapatrie jusqu'à 5000 exemples + parse JSON), pas du rendu DOM — ce n'est donc pas un chantier « virtualization ». Item conservé uniquement pour la taille du fichier : friction de lisibilité pour les contributeurs, pas de perf utilisateur.
...
- ~~**C-1, C-2, C-3 — Tests Vitest**~~ ✅ **Fait** : C-1 (fingerprint + persistence curation : `curationFingerprint.test.ts`, `curationReview.test.ts`), C-2 (normalisation d'import + parsing CoNLL-U : `screens/__tests__/ImportScreen.test.ts`, 32 tests), C-3 (diff LCS : `lib/diff.ts` extrait + `diff.test.ts`, 21 tests). Les fonctions ciblées étaient déjà extraites en exports nommés ; les trois suites de tests existent et passent.
...
- **Workflow visualization** entre écrans : ✅ **partiellement fait post-v0.2.3** — bandeau « étape suivante » contextuel après chaque action réussie (cf. Tier A #3). Reste optionnel : un fil d'Ariane d'ensemble *persistant* (strip d'état Import → Segment → Curate → Align → Export sur le hub) montrant en permanence où on en est. Non retenu pour cette itération — le bandeau contextuel adresse déjà la friction « et après ? ».
- ~~**Etat « index FTS périmé »** affiché sur chaque doc dans MetadataScreen~~ ✅ **Fait post-v0.2.2** : chip « ⚠ Index » ambré dans la colonne statut, dérivé via `indexer.stale_doc_ids`.
...
<!-- AnnotationView : à conserver. Étape finale efficace (relecture/vérification
     rapide avant concordancier). Le temps passé court = signal positif de
     workflow efficient, pas désengagement. Toutes les frictions du pipeline
     sont en amont (Import/Segmentation/Curation) — c'est là que doivent aller
     les efforts de réduction de friction, pas vers la suppression d'écrans
     finaux qui marchent bien. -->
