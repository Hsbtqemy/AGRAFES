...
| **SegmentationView** | 1595 | Mature ; **en cours de retrait au profit du canvas** (voir `docs/DESIGN_segmentation_retirement.md` — voie a) | Filtres anomalies (segments courts + ponct. orpheline). Undo Mode A (merge/split) **présent** (post-U-02). Onglet « Rôles » délégué à `components/RolesPane.ts` (logique pure dans `lib/conventionsRoles.ts` + `lib/conventionsUnitList.ts`). Le structure matcher (`SegStructureMatcherPanel`, inter-doc) est le dernier bloqueur — décomposé dans la note. | Élevée |
| **CurationView** | — | **Retirée (R6.5-C)** — la curation est désormais une **couche du canvas** (`components/CurationPane.ts`) | Parité au canvas atteinte en R6.5-B (exceptions/relu/Find-Replace/undo/toggle non_traduit) avant le retrait du legacy. CSS mort résiduel à purger en fin de migration (voir mémoire de report). | — |
