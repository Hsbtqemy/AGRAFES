...
| P1 | Tauri fixture expansion to GUI-capable runners | Headless smoke exists; full app-run coverage is still partial | Add optional GUI-capable runner lane that executes `tauri run`/`tauri build` smoke with sidecar | **done** (2026-04-09 : workflow `tauri-e2e-fixture` enrichie avec lane `fixture-gui-smoke` optionnelle (`workflow_dispatch` `run_gui_smoke=true`) sur macOS/Linux/Windows, avec download sidecar artifacts + `tauri build --debug`) |
...
| P2 | CI cache strategy for PyInstaller builds | Matrix builds can be slow/costly | Cached dependency/build layers with measurable time reduction | **done** (2026-04-09 : `setup-python` cache pip + cache `build/sidecar_pyinstaller` + build `--no-clean` sur `build-sidecar.yml`, `release.yml`, `tauri-e2e-fixture.yml`) |
| P2 | Sidecar binary size optimization | Onefile convenience increases binary size | Track size budget and apply PyInstaller/payload optimizations with before/after report | **done** (2026-04-09 : garde-fou `scripts/check_sidecar_size_budget.py` + budget `bench/fixtures/sidecar_size_budget.json` branché dans les workflows sidecar ; build PyInstaller optimisé (`--strip` hors Windows, `--optimize 1`, exclusions modules lourds) ; couverture tests `tests/test_sidecar_size_budget.py`) |
| P2 | Tauri release icons (icns/ico) | Dev currently uses PNG-only placeholders to unblock `tauri dev`; release packages need platform-native icons | Generate and validate real `icon.icns` + `icon.ico` assets before release packaging | **done** (2026-04-09 : `tauri-app/src-tauri/icons/` régénéré via `tauri icon` depuis source 1024 ; `tauri.conf.json` inclut de nouveau `icons/icon.icns` + `icons/icon.ico`) |
