...
| T-05 | 🟠 | — | 🟦 partiel | E2E Tauri *driver* (Playwright/WebDriver) toujours absent. Couverture d'intégration accumulée sans driver : render-smoke DOM happy-dom côté shell (`styleRegistry`/`diagnostics`/`telemetry`, PR #61→#63) ; **tests d'intégration de la couche connexion** `sidecarClient` (prep) — `ensureRunning` reuse portfile + in-memory, `shutdownSidecar`, `getActiveConn`/`resetConnection`, helpers URL — via mocks des plugins Tauri (PR ci-dessous), qui **gatent la réconciliation connexion d'U-01 (PR2)**. Reste : vrai driver E2E + chemin spawn (`_spawnSidecar`). |
