...
| IMP-09 ✔ | 🟠 | P1 | 🟦 partiel | Mauvaise extension → import charabia sans échappatoire. **Fix (échappatoire)** : `modeOptionsForExt` offre aux extensions TEXTE (`.txt`/`.conllu`) les modes texte en alternative (défaut détecté en 1er) → un CoNLL-U/TEI nommé `.txt` se re-route dans le menu par-fichier ; binaires (.docx/.odt = zip non ambigu) inchangés ; intitulé du menu explicite. Tests `importDetect.test.ts` (RED prouvé). *Reste (dette)* : pas de **reniflage de contenu** — l'échappatoire est manuelle, la mauvaise détection auto reste possible jusqu'à correction. `importDetect.ts`. |
| IMP-10 ✔ | 🟠 | P1 | 📝 documenté | Famille mal liée cross-dossier / mots-outils (`de/la`). **Vérif (2026-07-20)** : le groupement **cross-dossier est une feature DÉLIBÉRÉE et testée** (`familyDetect.test.ts:55`, cas ShareDocs chemins distincts) → le faux-positif `2020/rapport_fr`+`2021/rapport_en` est **indiscernable** du cas légitime par le nom seul ; scoper au dossier casserait la feature. Mots-outils : idem (`de`/`la` sont de vrais codes). Atténué par la **bannière éditable** (l'utilisateur confirme la famille) + IMP-13 (sous-cas même-langue). Limite intrinsèque de la détection par-nom → dette assumée. |
| IMP-13 | 🟡 | P1 | ✅ corrigé | Famille sans check langues distinctes. **Fix** : `detectFamilyGroups` exige **≥ 2 langues distinctes** (la docstring le promettait déjà, le code ne testait que `>=2` fichiers). Test `familyDetect.test.ts` (RED prouvé : deux `book_en` cross-dossier se regroupaient). `familyDetect.ts:66`. |
| IMP-03 | 🟡 | P2 | ✅ corrigé | CoNLL-U : aperçu tolère Latin-1, import strict le rejette. **Fix** : `preview_conllu` renvoie `not_utf8` (**contrat 1.6.61**, additif) ; le front avertit dans le résumé d'aperçu (« ⚠ non-UTF-8 — l'import rejettera »). Plus de « aperçu OK puis échec ». 2 tests. `conllu.py`. |
| IMP-04 | 🟡 | P2 | ✅ corrigé | TXT encodage. **Fix (b)** : le `try` autour de `charset_normalizer.from_bytes` catch `Exception` (pas seulement `ImportError`) → une erreur runtime ne crashe plus l'import (repli cp1252/latin-1). **Fix (a)** : compter les U+FFFD que `errors="replace"` masque + **warning** (couvre BOM/charset-normalizer que le warning `enc_method` ignorait). 2 tests (RED prouvé sur le crash). `txt.py`. |
| IMP-11 ✔ | 🟠 | P2 | ✅ corrigé | Langue devinée `fr` en silence. **Fix** : flag `langGuessed` (nom sans code de langue, hors TEI) → **bordure ambre + tooltip** « langue par défaut, vérifiez » sur le champ langue, effacé dès que l'utilisateur l'édite. Câblage `ImportScreen` (écran IO **non testé** — primitives `detectLanguageToken`/`isKnownImportExt` testées ; build tsc vert). |
| IMP-12 | 🟡 | P2 | 📝 documenté | Famille rate l'original non-marqué (`book.docx`+`book_en.docx`). **Décision (2026-07-20)** : ne PAS auto-détecter — un fichier sans token est ambigu (page de garde/brouillon → fausse famille) et l'inclure exigerait de **deviner sa langue** (report du problème IMP-11). Cohérent avec IMP-10 (limite intrinsèque du nom-seul). Reliage manuel via MetadataScreen. `familyDetect.ts:49`. |
| IMP-15 | 🟡 | P2 | ✅ corrigé | Drop local sans filtre d'extension (divergence ShareDocs). **Fix** : le handler de drop filtre via `isKnownImportExt` (converge avec ShareDocs) + toast des ignorés → un `.doc`/`.pdf`/sans-ext n'entre plus dans la liste avec un mode bidon. `ImportScreen.ts`. |
...
> → **les 3 corrigés depuis** : FE-02 (P1-9), FE-05 (P1-12), FE-03/04 (PR #186, QA shell 2026-06-29).
...
| SID-05 | 🟠 | P0-6 | ✅ corrigé | Provenance ShareDocs désormais **atomique**. `dispatch_import` gagne un paramètre `source_path` (défaut `None` → chemin local, **imports locaux inchangés**) propagé aux **7 importeurs**, qui écrivent l'URL distante directement dans leur `INSERT INTO documents` — **dans la transaction de l'import**. `_process_one` passe `source_path=entry.href` et **supprime le 2ᵉ `UPDATE`+commit** → plus de fenêtre de crash laissant `source_path = tmp_path`. Le « différé » du lot P0 (threader l'URL aux 7 importeurs) est donc levé. Tests : `test_import_remote_batch.py` — override stocké vs chemin local par défaut ; **atomicité** via un proxy de connexion simulant un crash au commit *suivant* l'import (RED sur l'ancien 2-phases → temp ; GREEN → URL). `dispatch.py` / 7 importeurs / `ingest.py`. |
| SID-06 | 🟠 | P1-13 | ✅ corrigé | **Volet auth** refermé en P0 (SID-02, `_WRITE_PATHS`). **Volet doc fermé** : les **12 routes / 14 opérations** servies mais absentes de l'OpenAPI (`/unit/context` déjà doc ; `/curate/exceptions[/set\|delete\|export]`, `/curate/apply-history[/record\|export]` en GET+POST, `/query/facets`, `/stats/lexical\|compare`, `/segment/delete_structure_unit`) sont **documentées** (paths + schémas req/resp inline, `security` token sur les mutatrices), + 4 schémas composants (`StatsSlot`, `LexicalStatsResult`, `CurateExceptionsListResponse`, `CurateApplyHistoryListResponse`). **Bonus** : 2 `$ref` pendants préexistants définis (`OkResponse` ×8, `AlignLinkCreateRequest` — même classe que SID-13). `openapi.json` + snapshot + `SIDECAR_API_CONTRACT.md` régénérés ; 72 tests contrat verts. |
...
| SEC-03 | 🟡 | P2-19 | ✅ corrigé (P3 ; smoke runtime fait + `devCsp`) | CSP ajoutée aux **3 apps livrables** (shell/prep/app) : `script-src 'self'` (durcissement XSS ; Tauri 2 nonce ses scripts, Vite = modules `'self'`), `style-src 'unsafe-inline'`, `connect-src` loopback+`ipc:`, `object-src 'none'`, `base-uri 'self'`. I/O sidecar = `@tauri-apps/plugin-http` (Rust) donc non gated par `connect-src`. **Fixture e2e hors périmètre** (non livré, `bundle.active:false`). **Maj 2026-06-29 (validation)** : (1) **smoke runtime fait** — `dist/index.html` des 3 apps chargé en **Chromium headless avec la CSP réelle posée en en-tête HTTP** → **0 violation** (les scripts module externes `'self'` chargent ; aucun `'unsafe-eval'` requis au démarrage). Analyse statique confirmante : sortie Vite = scripts module externes + `<style>` inline seulement ; **0 gestionnaire d'événement inline** (`on*=`) dans les 3 fronts (tout en `addEventListener`). (2) **`devCsp` ajouté** aux 3 confs (clé **confirmée valide** dans `@tauri-apps/cli/config.schema.json` — l'hésitation P3 est levée) : relâche `script-src` (`'unsafe-inline' 'unsafe-eval'`) + ajoute `ws://127.0.0.1:* ws://localhost:*` à `connect-src` pour le HMR Vite — **dev-only, jamais embarqué**. **Résidu** : la webview Tauri réelle (injection de nonce, protocoles `ipc:`/`asset:`) + chemins de code paresseux (ex. Chart.js lazy) non exercés headless → vrai pilotage = **T-05** (driver e2e). |
...
| SEC-05 | 🟢 | P2-19 | 📝 documenté (risque accepté) | `register_sidecar` (Rust) fait confiance à `base_url` JS pour `/shutdown` (théorique : appelant déjà détenteur du token). **Non corrigé** : `main.rs` n'est **validé par aucune CI de PR** (build Rust shell tag-only) → un correctif à l'aveugle risquerait de casser le build sans filet, pour un 🟢 loopback-only. À traiter si/quand une CI Rust (cargo build/clippy) est ajoutée, ou pendant un cycle de release. `main.rs:262`. |
| SEC-06 | 🟢 | P2-19 | 📝 documenté (risque accepté) | `sidecar_fetch_loopback` recopie les headers JS sans allowlist (host loopback → impact ~nul). **Non corrigé** pour la même raison que SEC-05 (Rust hors CI PR). `main.rs:59`. |
| SEC-07 | 🟢 | P2-19 | ✅ corrigé (P4) | `serve --token <littéral>` ne persiste plus le secret dans `runs.params` : helper `_safe_token_label(token_mode)` → `auto`/`off`/`custom` (le littéral devient `custom`). Les autres usages du token (bool `token_required` en run-stats ; token résolu renvoyé sur stdout) sont intentionnels/sûrs. Test : `test_cli_contract.py::test_serve_token_label_redacts_literal_secret`. `cli.py`. |
...
| FE-06 | 🟡 | P2-18 | 🟦 partiel (point d'arrêt) | (= U-03) Campagne de couverture `tauri-app` — **tout l'extractible à logique pure est testé** (PR #179→#183) : `features/stats.ts` (rendu compare + parseur de filtres), `ui/results.ts` (rendu KWIC/parallèle, dont **anti-XSS du highlight** prouvé RED-sur-cassé), `features/docSelector.ts` (persistance localStorage + checklist), `ui/dom.ts` (`escapeHtml` primitive anti-XSS + `elt`). **`tauri-app` 81 → 128 tests.** **Point d'arrêt assumé** : le reste (`features/metaPanel.ts`, `features/importFlow.ts`) est du rendu + I/O sidecar → faible ROI en test unitaire (demanderait un render-smoke avec mocks = ticket dédié si besoin). |
...
| ID | Sév | Prio | Statut | Constat (résumé) |
|----|-----|------|--------|------------------|
| OPS-01 | 🟠 | P1-10 | 📝 documenté | Growth-gate non bloquant **par choix documenté** (`::warning::` + commentaire : les PR de réduction A-01 passaient elles-mêmes au rouge, fenêtre 90 j dominée par la croissance historique). À re-durcir (`exit 1`) une fois le net 90 j confirmé sous le seuil — décision de politique CI. **Maj 2026-08-25** : seuil relevé 500 → **2500**, avec `THRESHOLD_REVIEW=2026-12-15` qui émet un rappel passé cette date. Motif mesuré : le net de 500 n'était atteignable que parce que l'extraction des 17-18 juin pèse −1436 dans la fenêtre glissante ; elle en sort le 16 septembre et le gate lirait **+1995 sans un seul commit de plus**. Un seuil intenable cesse d'alerter. A-01 reste ouvert (`différé`), et sa conclusion mesurée est que la reprise doit être une **scission par domaine**, pas une extraction de plus. `sidecar-growth-gate.yml`. |
...
| U-03 | 🟡 | P2-11 | 🟦 partiel (point d'arrêt) | `tauri-app` : couverture étendue via **FE-06** (#179→#183) — `search.ts` (#60) + `stats.ts`, `ui/results.ts` (anti-XSS), `docSelector.ts`, `ui/dom.ts` (`escapeHtml`/`elt`) → **81 → 128 tests** (happy-dom, gatés CI). Reste `metaPanel.ts`/`importFlow.ts` = rendu/IO (point d'arrêt, cf. FE-06). |
...
| F-03 — épinglage GitHub Actions par SHA | 2026-04-19 | ✅ corrigé | **B** (PR #166) : 7 occ. **tierces** au SHA (`dtolnay/rust-toolchain` ×3 + `with: toolchain: stable` ; `softprops/action-gh-release` ×2 ; `Swatinem/rust-cache` ×2 ; workflows tag-only). **A** (empilé) : les 6 `actions/*` first-party (**77 occ.**) au SHA — exercées par la CI PR (`ci.yml`/`smoke.yml`). Cadrage + SHA : `docs/TICKET_F03_SHA_PINNING.md`. Dependabot couvre déjà `github-actions`. |
