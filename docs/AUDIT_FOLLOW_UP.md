...
| N-04 | 🟡 | P2-14 | 🟦 arbitré (ADR-043) | Resegmentation écrase `text_raw` + supprime les liens. **Arbitré ADR-043** : colonne `text_source` immuable (préserve l'original d'import) ; implémentation phasée P1 (migration 020 + import peuple) → P2 (resegment/merge/split propagent) → P3 (UI). Suppression des liens hors périmètre. |
