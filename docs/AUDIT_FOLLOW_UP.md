...
|----|-----|------|--------|------------------|
| A-01 | 🔴 | P0-1 | 🟦 partiel | `sidecar.py` monolithe (9961→~8800 l). Couche `services/` : `import` (#46) + `conventions` (#50) + `doc_relations` (#52) + `documents` (#53, +`BadRequestError`) + `curate`-CRUD (#54) + `units` (set_role/bulk/update_text/list ; merge/split laissés). Handlers→adapters fins (lock writes, map erreurs typées→codes, envelope ; byte-identiques). Couplé laissé en adapter (assumé) : backfill schéma `_ensure_*`, télémétrie `doc_deleted`. Filet : smoke-run binaire en CI (#51) = 1 GET/couche. **Écarté** : `export` (lock incohérent par handler + valeur faible, format déjà dans `exporters/`). Restants : `query`/`token`/`stats` (surtout délégateurs) ; laissés : `units/merge`+`split`, `curate_preview`/`*_export`, `align`/`segment`/`jobs` (à état/couplés). |
