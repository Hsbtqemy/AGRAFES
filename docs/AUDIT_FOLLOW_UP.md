...
| FE-06 | 🟡 | P2-18 | 🟦 partiel | (= U-03) Campagne de couverture `tauri-app` **en cours**. **Testés** (PR #179/#180/#181) : `features/stats.ts` (panneau stats — rendu compare, parseur de filtres), `ui/results.ts` (rendu KWIC/parallèle, dont **anti-XSS du highlight**), `features/docSelector.ts` (persistance localStorage + checklist). **`tauri-app` 81 → 120 tests.** Restent : `features/metaPanel.ts`, `features/importFlow.ts`, et le gros `ui/dom.ts` (1788 l.). |
...
| U-03 | 🟡 | P2-11 | 🟦 partiel | `tauri-app` : couverture étendue via la campagne **FE-06**. Au-delà de `features/search.ts` (#60), tests directs (happy-dom, gatés CI) ajoutés sur `stats.ts`, `ui/results.ts` (dont anti-XSS), `docSelector.ts` (PR #179→#181) → **81 → 120 tests**. Reste : `metaPanel.ts`, `importFlow.ts`, `ui/dom.ts` (1788 l.). |
