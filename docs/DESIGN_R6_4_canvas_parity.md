...
- **R6.5-A — Retrait Annotation** ✅ *(livré 2026-07-20 ; le petit écran — prouve le playbook)*. **#22 recadré** (token-search *pour éditer*, §7.5 — commit `9421308`) · **#23 relogé** (pont de correction Explorer→Prep → canvas, `af08f51`) · **#24 abandonné** (7.2) · **retrait** : `AnnotationView.ts` supprimé (−832 l.), sous-vue `annoter` + `_annotationView` + `_renderAnnoterPanel` + `annotFocusDoc` (devenu mort) + `refreshIfConnected` + carte hub / `dispose` / CSS `.actions-sub-annoter` morts. **Audit du retrait (§7.6)** : la sous-vue `annoter` était aussi une **destination de navigation** (hub, sidebar, « étape suivante → annoter » depuis Segmentation [vivant] + Curation, callback `onNavigate`) → **re-routées vers le canvas** (`ActionsScreen.openAnnotationLayer` → `TextCanvasView.showAnnotationLayer`/`focusAnnotationToken`), **zéro régression de nav**. Build + 1119 tests verts. **Différé** : pont inverse Prep→Explorer (§7.5).
...
### 7.5 Recadrage #22 (2026-07-20) — outil d'édition, pas de recherche

**Prémisse corrigée (vérifiée au code).** §2 classait #22 (recherche token) en 🔴 « vraie perte » en supposant le concordancier incapable de chercher par lemme/UPOS. **Faux** : le concordancier a le mode **CQL** (`POST /token_query` `tauri-app/sidecarClient.ts:411` ; `builderMode:'cql'` `state.ts:65` ; prédicats `word|lemma|pos|upos|xpos|feats` `search.ts:140` ; ex. `[pos="DET"][lemma="liv.*" %c]`), **cross-corpus + KWIC**. La note roadmap R4.3 « token_query (CQL) différé » est **périmée** (dérive de backlog).

**Mais le concordancier _lit_ ; il ne _modifie pas_.** La modification des annotations est le métier de **Prep** (éditeur de token R5.2d). Donc #22 n'est ni une perte de *recherche* (l'Explorer la fait mieux) ni un pur doublon : c'est un **« chercher-pour-corriger »** propre à Prep. Deux directions à tenir :
- **Explorer → Prep (corriger)** = deep-link **#23** `annotFocusDoc(docId, tokenId)` : d'un hit CQL, sauter sur le token dans Prep, éditeur ouvert. *Pont de correction — il bloque le retrait tant qu'il route vers le legacy.* À reloger au canvas.
- **Dans Prep (passe de vérif locale)** = **#22 recadré** : la recherche de la pane Annotation = token-search (mot/lemme/UPOS) + surlignage (`[data-token-id]` déjà porté par la prose **et** l'interlinéaire) + ◄►/compteur ; **clic sur une occurrence = éditeur** (déjà câblé). Remplace le filtre d'unités *dans cette pane seule* (les autres panes gardent le leur). Une passe « tous les VERB de ce doc, je corrige les faux » **sans rebondir vers l'Explorer** à chaque fois.

**Reclassement** : #22 = 🔴 **porté (recadré édition)**, pas abandonné. **Différé** : pont inverse **Prep → Explorer** (« chercher ce doc dans le concordancier », la direction *recherche*) — hors périmètre retrait, petite feature future.

### 7.6 Audit du retrait Annotation (2026-07-20) — la sous-vue était une destination de nav

Le retrait de l'écran legacy s'est révélé plus intriqué que « supprimer le fichier » : la sous-vue `annoter` n'était pas qu'un panneau mais une **destination de navigation** référencée par des entrées **vivantes** —
- carte hub « Annotation » (`actionsHubTemplate`, `data-target="annoter"`),
- lien sidebar « Annotation » (`app.ts`, `data-nav="annoter"`),
- « étape suivante → Annoter » depuis **Segmentation** (`SegmentationView`, écran **vivant**) et Curation (legacy), via le callback `onNavigate`.

**Décision — re-route, pas suppression des entrées** (choix utilisateur) : l'annotation étant désormais une **couche du canvas**, ces entrées ouvrent le canvas (sous-vue `texte`) sur la couche Annotation. Nouveau point d'entrée `ActionsScreen.openAnnotationLayer(docId?)` → `setSubView("texte")` puis `TextCanvasView.focusAnnotationToken(docId)` (doc-scopé) ou `showAnnotationLayer()` (doc courant). Les **3 mécanismes de dispatch** (hub `data-target`, sidebar `data-nav`, `onNavigate`) interceptent `annoter` avant `_switchSubViewDOM` → **zéro entrée de nav perdue**.

`prepNextStep` garde `annoter` comme cible (jamais émise par `computeNextSteps` aujourd'hui, mais forward-compatible : `onNavigate("annoter")` route au canvas). Modules partagés (`annotationProse`, `annotationSpacing`, `models`, `jobPolling`) **conservés** — utilisés par la pane canvas. Blast radius **confiné à `tauri-prep`** (aucune réf shell/Python), conforme au §7.4.
