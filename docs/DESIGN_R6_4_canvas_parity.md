...
- **R6.5-A — Retrait Annotation** *(le petit ; prouve le playbook)*. **#22 recadré** (token-search *pour éditer*, cf. §7.5) : la recherche de la pane Annotation devient token-search (mot/lemme/UPOS) + surlignage + ◄►/compteur, clic → éditeur ; remplace le filtre d'unités *dans cette pane* → reloger **#23** (`annotFocusDoc` route vers le canvas — *pont de correction Explorer→Prep, essentiel*) → **#24** abandonné (7.2) → **retrait** : sous-vue `annoter`, `_annotationView`, `_renderAnnoterPanel`, carte hub (`lib/actionsHubTemplate.ts`), `dispose`, lien sidebar (`app.ts:365`), token-nav RG→Prep (`app.ts:167-168`). **Différé** : pont inverse Prep→Explorer (§7.5).
