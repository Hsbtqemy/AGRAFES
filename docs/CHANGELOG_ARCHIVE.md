# Changelog — archive (historique pré-consolidation)

Entrées sorties du changelog courant pour le garder court : la **série semver [0.1.x]**
(0.1.12 → 0.1.44, archivée à l'OPS-05), le schéma de versions « V… » (V0→V1.9) et les
incréments initiaux. Le changelog **courant** (`[Unreleased]` + série `[0.2.x]`) est dans
[`../CHANGELOG.md`](../CHANGELOG.md). Archive append-only (findings D-02, OPS-05).

---

## Série 0.1.x (semver post-consolidation, archivée — OPS-05)

> Séries 0.1.12 → 0.1.44, sorties du changelog courant pour le maintenir court.
...
## [Pré-consolidation — sprints non versionnés]
