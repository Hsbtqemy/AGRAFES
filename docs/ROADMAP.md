...
- **N-04 — resegmentation destructive : ✅ arbitré (ADR-043)** : décision = colonne **`text_source` immuable** (préserve le verbatim d'import, jamais réécrit par curate/resegment/merge/split). Implémentation **phasée** : **P0** centraliser `insert_units` (helper partagé, 7 importeurs + segmenter/undo dédupliqués — débloque P1 en 1 point au lieu de 7) ; **P1** migration 020 + import peuple (`= text_raw`, lignes existantes `NULL`) ; **P2** resegment/split/merge propagent (héritage/concat) ; **P3** UI (afficher/exporter l'original). La suppression des `alignment_links` reste hors périmètre (couverte par warning + Mode A undo). Cf. `docs/DECISIONS.md` ADR-043.
