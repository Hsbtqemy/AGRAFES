...
- **S-03 — burndown XSS lint** (P0-2 résiduel) : ~~job ESLint bloquant~~ **✅ fait (phase 1)** ; ~~AlignPanel~~ **✅ fait (géant 1/4, baseline 89 → 72)**. Reste : résorber les 72 sites *grandfathered* restants (Curation 31, Segmentation 21, Metadata 20 — par écran, avec approfondissement de l'échappement, via `npm run lint:prune`) + créer la config no-unsanitized pour app & shell (phase 2).
