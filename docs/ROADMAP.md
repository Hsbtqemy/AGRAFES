...
- **ADR-043 / N-04 — `text_source` immuable (clos, #111→#118)** : colonne `units.text_source` préservant le texte d'import verbatim contre la resegmentation destructive. 8 PR : P0 `insert_units` centralisé (#111) ; P1 migration 020 + peuplement import (#112) ; P2 resegment + undo (#113) ; P2b merge/split + undo, migration 021 (#114) ; P3a lecture (`/units`, `/documents/preview`) + export `source_field=text_source`, contrat 1.6.30 (#115) ; P3b repli inline « voir l'original » + sélecteur export (#116) ; P2c propagation = limite documentée (#117) ; clôture registre findings (#118). Cf. `docs/DECISIONS.md` ADR-043.
- **ShareDocs/WebDAV — Phases 5-6 mergées (#106/#107)** : P5 détection format+langue par fichier via `importDetect` partagé (+ expansion PROPFIND des dossiers cochés, convergence TEI `xml:lang` local) ; P6 familles `translation_of` câblées à l'import par lot via `familyDetect` + bannière hybride. **ShareDocs complet P1→P6.** Cf. `DESIGN_sharedocs_ingestion.md` §11/§12.
...
- **WebDAV/ShareDocs — Phase 1** : sous-commande CLI `import-remote` (client stdlib, dédup, dispatch unifié). *(Suite : P2→P6 toutes mergées dans `dev` — ShareDocs complet, voir ci-dessus.)*
...
- **★ Refonte UI « canvas texte » + capacités corpus (R1→R6)** — plan consolidé des 3 notes de design dans [`ROADMAP_REFONTE.md`](ROADMAP_REFONTE.md) : colonne vertébrale = le canvas, **frontière moteur/front explicite** par item (§4). **R1 en cours** (prototype canvas + conscience du stade A0/R1.1, sur WORKCOPY, **non commité**).
...
- **Analytics Concordancier — distribution temporelle : ✅ complet (F2a+F2b)** : phase 1 (tri KWIC + dispersion) + collocations (`/token_collocates`) + **`token_stats` `group_by="year"`** (F2a : bucket année du `doc_date`, comptage par hit, comptes bruts **et** `freq_per_10k` normalisée ; contrat 1.6.31) + **histogramme Chart.js** dans le module Recherche du shell (F2b : option « Année », barres verticales chronologiques, bascule Occurrences/Fréquence, helper pur `yearChart.ts` + 7 tests). **Volet analytics du concordancier clos.** Cf. `docs/cadrage/DISTRIBUTION_TEMPORELLE.md`.
- **Dette front (U-02 — point d'arrêt atteint)** : campagne de dégraissage des écrans Prep **menée à son plancher à comportement-préservant** (vagues #139→#147 puis #149→#157, ~−279 l. en vague 2) — `CurationView` 3802→3087, `MetadataScreen` 3201→1979, `SegmentationView` 2671→1595, + ImportScreen/ShareDocs/ExportsScreen/ActionsScreen/AnnotationView/AlignPanel ; ~30 modules `lib/` testés extraits, 3 vraies duplications supprimées, 3 faux dedups évités (byte-identité). **Socle de dé-risquage** qui l'a permis : infra **render-smoke happy-dom pour `tauri-prep`** (opt-in par docblock) + tests de caractérisation montant `CurationView`/`MetadataScreen` sans sidecar (T-05 étendu app/shell→prep). **Reste = tickets dédiés** (hors U-02) : cœurs interactifs (éditeur bitext AlignPanel, raw-pane/conventions/manual-override CurationView, form-lifecycle `_renderEditPanel` MetadataScreen, `_renderSegSavedTable` SegmentationView) — conception data-ownership/seam à part ; + finding sécurité `roleBadge` (drift à 3 variantes → *hardening*). Couverture Vitest large `tauri-app` (U-03). Reste hors filet : vrai driver E2E (Playwright/WebDriver).
- **Finitions & décisions UX (mineur)** : ~~coloration syntaxique du champ CQL~~ **✅ fait** (overlay calque coloré derrière textarea transparent ; tokeniseur pur `cqlHighlight.ts` + 8 tests ; cosmétique) ; SP-2.1 — navigation pivot→Prep **actée comme voulue** (cohérent avec F3), éventuel +deep-link Explorer optionnel ; D3 — harmoniser le badge « Source modifiée » dans le concordancier shell (optionnel).
