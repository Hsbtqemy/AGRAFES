...
- **WebDAV/ShareDocs — Phases 2-4 ✅ livrées dans `dev`** (#103 sidecar + UI Prep, #104 P4A-C trousseau/navigation/sélection, #105 P4D sécurité+annulation). **Phase 5 — détection par fichier ✅ livrée** (branche `feat/sharedocs-p5-detection`) : format + langue par fichier via le module partagé `importDetect` (source unique import local + ShareDocs), groupement par `(parent, mode, langue)`, glob retiré — §11 + §11.8. **Suivi P5 ✅ livré** (même branche) : expansion PROPFIND des **dossiers cochés** dans « Importer la sélection » (non récursif, dédup par href, erreur par dossier non bloquante — `routeEntriesToImport`/`dedupeDetectedFiles`) **+** convergence de l'**import local** sur le `xml:lang` TEI (helper partagé `detectLanguageForMode`). **Phase 6 — familles à l'import ShareDocs : cadrée, plan en 2 tickets** (branche `feat/sharedocs-p6-families`) — câbler `translation_of` à l'import par lot, comme l'import local (le système familles est déjà entièrement construit, cf. `BACKLOG_FAMILLES.md` ; seul le point d'entrée ShareDocs manque). Front-only. **P6A** (`TICKET_SHAREDOCS_INGESTION_P6A_FAMILY_DETECT_SHARED.md`) : extraire `detectFamilyGroups` en module partagé `familyDetect`. **P6B** (`TICKET_SHAREDOCS_INGESTION_P6B_FAMILIES.md`) : détection + bannière hybride (pivot pré-sélectionné modifiable) + barrière de complétion de lot + câblage post-import. Réf. `DESIGN_sharedocs_ingestion.md` §12.
