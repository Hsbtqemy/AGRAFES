...
- **WebDAV/ShareDocs — Phase 1** : sous-commande CLI `import-remote` (client stdlib, dédup, dispatch unifié). *(Suite : P2-P4 livrées dans `dev` — voir « Court terme » ; P5 conçue.)*
...
- **WebDAV/ShareDocs — Phases 2-4 ✅ livrées dans `dev`** (#103 sidecar + UI Prep, #104 P4A-C trousseau/navigation/sélection, #105 P4D sécurité+annulation). Prochaine tranche : **Phase 5 — détection par fichier** (format + langue, réutilise la détection de l'import local via un module partagé `importDetect` ; **conçue, non implémentée** — §11 + `TICKET_SHAREDOCS_INGESTION_P5_DETECTION.md`) ; puis **Phase 6 — familles** (lien source↔traduction, différé). Réf. `DESIGN_sharedocs_ingestion.md`.
