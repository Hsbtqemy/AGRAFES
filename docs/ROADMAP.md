...
- **Analytics Concordancier — distribution temporelle : backend ✅ (F2a)** : phase 1 (tri KWIC + dispersion) + collocations (`/token_collocates`) + **`token_stats` `group_by="year"`** (distribution diachronique : bucket année du `doc_date`, comptage par hit, comptes bruts **et** `freq_per_10k` normalisée ; contrat 1.6.31) **livrés**. **Reste F2b** : rendu histogramme temporel dans l'UI stats du concordancier. Cf. `docs/cadrage/DISTRIBUTION_TEMPORELLE.md`.
