...
- **Dette front** : `CurationView`/`SegmentationView`/`AnnotationView` **déjà extraits** d'`ActionsScreen` (U-02 partiel) ; les vues résultantes restent volumineuses (Curation 3802 l. / Metadata 3201 l.) → découpe à venir. **Socle de dé-risquage posé** : infra **render-smoke happy-dom pour `tauri-prep`** (opt-in par fichier via docblock, env `node` par défaut préservé) + tests de caractérisation montant `CurationView` **et** `MetadataScreen` sans sidecar (T-05 étendu de l'app/shell vers prep). La décomposition de ces deux monolithes peut désormais s'appuyer sur ce filet. Couverture Vitest large `tauri-app` (U-03). Reste hors filet : vrai driver E2E (Playwright/WebDriver).
