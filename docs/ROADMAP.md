...
- **S-03 — burndown XSS lint** (P0-2 résiduel) : ~~job ESLint bloquant (phase 1)~~ ; ~~les 4 écrans géants prep (AlignPanel, Metadata, Segmentation, Curation)~~ **✅ TOUS faits — baseline 89 → 0, `eslint-suppressions.json` supprimé, garde pleinement stricte**. Reste (phase 2) : créer la config no-unsanitized pour `tauri-app` & `tauri-shell` (aucune aujourd'hui).
