...
- **S-03 — burndown XSS lint** (P0-2 résiduel) : ~~job ESLint bloquant~~ **✅ fait (phase 1)** ; ~~AlignPanel~~ **✅ (1/4)** ; ~~Metadata~~ **✅ (2/4)** ; ~~Segmentation~~ **✅ (3/4)** — baseline 89 → **31**. Reste : **Curation 31** (dernier géant — porte aussi le bug couleur de rôle brute `conv.color` L2920, via `safeColor`) + créer la config no-unsanitized pour app & shell (phase 2).
