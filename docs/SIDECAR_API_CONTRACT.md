...
- `POST /families/{family_root_id}/align` — align all parent↔child pairs in a family (token required)
  - **`target_doc_ids` (1.6.77, facultatif)** — n'aligner que ces enfants ; omis = toute la famille. Ce n'est pas un filtre d'affichage mais le **périmètre de destruction** : avec `replace_existing`, la purge est faite paire par paire, donc une colonne hors périmètre garde ses liens — y compris ceux posés à la main (`status IS NULL`), que `preserve_accepted` ne sauve pas. Le filtre est posé **avant** le contrôle de segmentation : une colonne hors périmètre n'est ni purgée, ni réalignée, ni rapportée « ignorée ». Liste vide ou enfant étranger à la famille = **400** (ALI-15).
