...
- [ ] Trancher le caractère **amnésique** de ce même signal — trouvé le 27 août en passe de QA. Il ne vit que dans un `Set` du front (`AnnotationPane._staleAnnot`), vidé à chaque chargement de document (`AnnotationPane.ts:183` et `:209`). Recharger fait disparaître la pastille alors que le texte, lui, a bel et bien changé : la surcouche se repeint, et l'annotation redevient muette sur son propre décalage. Rien en base ne permet de le retrouver — `tokens` n'a ni date ni run parmi ses dix colonnes, et `units.text_source` n'est pas alimenté par ce chemin : le document 387 compte **11 actions `update_text`** et **0 unité** au `text_source` renseigné. Reconstituer le signal demanderait de comparer la date de la dernière édition (`prep_action_history`) à celle de l'annotation — qui n'est enregistrée nulle part. Conséquence de bord sur RICH-01 : le geste de stylisation, refusé sur une ligne repeinte, redevient possible sur la même ligne tant que la session la sait périmée, puis impossible après un rechargement
