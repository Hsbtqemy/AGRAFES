...
- [x] Poser un style ne refait plus la liste entière — **fait le 27 août**, sur un signalement de QA (« la stylisation est assez lente en vue Étendue »). `_applyStyle` appelait `render()` : sur `Lodge-Small_ES.docx`, une balise posée sur un mot faisait reconstruire 1754 lignes **et** les surcouches de tokens que la couche y repeint, 20 636 en tout — alors que le geste ne change que `text_raw`, dont ni la recherche, ni les compteurs, ni les blocs ¶ ne dépendent. Remplacé par un repeint de la seule ligne visée, dont le contenu est celui-là même que le rendu produit. Au passage, les deux boucles de câblage des lignes cherchaient chaque unité par balayage de `_units` : 3,1 millions de comparaisons par rendu sur ce document, et deux fois — passées par un index. Test posé (rouge sur l'ancien code) : les nœuds voisins survivent au geste et `decorateRow` n'est pas rappelé
