...
**Où se joue la zone « ce que l'application doit montrer sans le taire ».** Trois endroits.
Le fichier irrégulier `2021_Texte6…- Copie.docx` est dans le **même dossier** que `Texte1` et
se joue à l'écran **Importer** puis dans l'**Alignement**. Le fichier à tableaux hétérogènes
est `HDR V7 06 juillet.docx`, à la **racine de `Downloads`** (ou
`Conventions-Textes journalistiques - Raluca-Pourétu-Juillet.docx`, dans
`00-Hugo-Corpus Multilingues`) : l'ajouter suffit, il n'y a **rien à importer** — on regarde
seulement ce que la carte d'aperçu en dit. Le dernier point est dans l'écran **Métadonnées**,
panneau d'audit.
...
- [x] Le journal termine chaque import par **« · réindexez pour la recherche. »** — sans ce geste le document existe mais reste introuvable, et rien ne le disait jusqu'ici
...
- [x] **Reconstruire l'index avant de chercher** — un document importé n'est jamais trouvable tant que l'index n'a pas été refait : `fts_units` est une table FTS5 **sans trigger**, peuplée par l'indexeur seul (migration 002). Ce n'est pas un défaut du lot, c'est le modèle
