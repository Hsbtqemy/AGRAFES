...
Si vous ne voyez aucun tableau sur `2021_Texte1` alors qu'il est bien sélectionné, c'est
le sidecar qu'il faut suspecter avant le code. Reconstruire avec `python scripts/build_sidecar.py --preset shell
--format onefile`, application fermée.

**Où ça se passe.** Écran **Importer**, **panneau de droite** — celui du fichier
**sélectionné** dans la liste. Le tableau y est sous la ligne de commandes, au-dessus de la
note de tableau et de l'extrait des unités. Plus rien à déplier : cliquer un fichier suffit.
**Rien n'est à importer dans cette passe** : on ajoute des fichiers, on regarde, on retire.

**Ce que le tableau affiche.** Une ligne par mode comparable — deux pour un `.docx` ou un
`.odt`, un seul pour un `.txt`. Colonnes : *Mode*, *Unités*, *Indexables*,
...
modes rendent **le même total d'unités** et ne se séparent que sur combien sont indexables.
...
qui compte le plus d’unités indexables. Sans quoi l'écran se contredirait : la carte du fichier
...
| fichier | mode | unités | indexables | non indexées |
|---|---|---|---|---|
...
- [ ] La ligne Paragraphes annonce **48 indexables** et **0 non indexées**
- [ ] La ligne Lignes numérotées annonce **0 indexables** et **48 non indexées**
...
- [ ] `9_CI-TrFr-2021_Aligné_UTF8.txt` affiche **48 unités, 0 indexables** et le même bandeau — c'est un `.txt` numéroté « 1. » et non `[1]`, aucun mode ne sait le lire aujourd'hui
...
- [ ] Sélectionner un **autre fichier** dans la liste recalcule le tableau pour lui — il n'y a plus de bouton « Suivant », c'est la sélection qui décide

### Non-régression

- [ ] Un `.docx` ordinaire (`8-CI-TrEn-2022_A Aligner.docx`) affiche ses deux lignes sans bandeau, et le reste du panneau se comporte normalement
...
- [ ] Un `.conllu` sélectionné n'affiche **aucun** tableau comparatif ni note de tableau : le panneau bascule sur l'aperçu des **tokens** (Phrase / ID / Forme / Lemme / UPOS), dans le même cadre
