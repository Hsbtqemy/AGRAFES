...
- [x] Même contrôle au **démarrage**, qui est le cas le plus grave parce qu'il ne demande aucun geste : renommer une base hors de l'application pendant qu'elle est fermée, rouvrir, et vérifier qu'on est averti plutôt que posé devant un corpus vide. Attendu : la bannière, le déclencheur à « (aucune) », et rien de recréé — le chemin est lâché plutôt que transmis aux modules. **Vérifier d'abord quelle base est persistée** : au premier essai du 3 septembre, l'application a rouvert sur `pas-une-base.db`, la base *illisible* de cette même passe, et a montré une bannière qui ressemblait à la bonne pour une cause opposée. Forcer la base voulue avant de conclure — `localStorage.setItem('agrafes.lastDbPath', …)` en reprenant le chemin depuis l'entrée grisée des récentes, puis recharger
